...
- `POST /api/scan/<id>/cancel` — 실행 중인 스캔 취소 (pending/running 상태에서만 가능)
...
```python
{
    ...
    "cancelled":      bool,         # 취소 플래그 — _run_scan 루프가 다음 모듈 전환 시점에 참조
    "results":        list,
    "risk":           dict | None,
...
| POST | `/api/extract/<id>/cancel` | `ctx.cancelled=True` 동기화로 안전 중단. Body `{reset: bool}` — `false`(기본): 현재 액션만 취소 후 ready 복귀 (누적 데이터 유지), `true`: 완전 종료 후 GC 대상 편입. 진행 중 액션 없으면 상태 무관하게 즉시 cancelled 마킹 |
